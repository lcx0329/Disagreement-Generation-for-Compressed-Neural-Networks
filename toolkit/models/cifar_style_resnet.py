...
class PadLayer(nn.Module):
    def __init__(self, padding, value) -> None:
        super().__init__()
        self.const = nn.ConstantPad2d(padding, value)
    
    def forward(self, x):
        return self.const(x[:, :, ::2, ::2])
    ...
    def __init__(self, in_planes, planes, stride=1, option='A'):
        ...
        if stride != 1 or in_planes != planes:
            if option == 'A':
                ...
                """
                self.shortcut = PadLayer((0, 0, 0, 0, planes//4, planes//4), 0)
            elif option == 'B':
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride, "B"))
